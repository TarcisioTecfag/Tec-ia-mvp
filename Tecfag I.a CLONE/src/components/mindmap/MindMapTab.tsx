...
import { useState, useEffect, MouseEvent, WheelEvent, useMemo, useRef, useCallback } from "react";
import { Plus, Trash2, Edit2, Save, X, Undo, Link as LinkIcon, Check, Settings, Zap, Cog, Box, Database, Cpu, Activity, AlertTriangle, User, Server, Wifi, Truck, ShoppingCart, ChevronDown, ChevronRight, ChevronLeft, GitFork, Pencil, Eye, EyeOff, Search, Maximize2, Minimize2, Wand2, Sparkles, Loader2, FileText, Network, ArrowLeft } from "lucide-react";
...
import { useToast } from "@/components/ui/use-toast"; // Fixed import path to align with other files if needed, or check hooks. ChatTab used components/ui/use-toast.
...
const MindMapTab = ({ isAdmin, initialMapId, onBack }: MindMapTabProps) => {
  ...
  // 2. Load Initial State (Prioritize props, then storage)
  useEffect(() => {
    const loadData = async () => {
      ...
      if (loadedFlowsStr) {
        try { loadedFlows = JSON.parse(loadedFlowsStr); } catch (e) { console.error(e); }
      }

      let combined = [...loadedFlows];

      // 2. Load API
      try {
        const apiMindmaps = await mindmapsApi.getAll();
        const apiFlows: Flow[] = apiMindmaps.map(m => ({
          id: m.id,
          name: m.name || "Sem Nome",
          nodes: m.nodes.map(n => ({
            ...n,
            connections: n.connections || [],
            typeId: (n.type as any) || 'process',
            collapsed: false
          })),
        ...
        // Merge: Prefer API for same IDs, add new ones (filter out locals with same ID)
        const nonApiLocals = loadedFlows.filter(lf => !apiFlows.find(af => af.id === lf.id));
        combined = [...nonApiLocals, ...apiFlows];
      } catch (error) {
        console.error("Failed to load API mindmaps", error);
      }

      // 3. Examples (ensure they exist)
      ...
      const exampleFlow3: Flow = {
        ...
        nodes: [
          ...
          // ... (truncated content for brevity, relying on user already having it or just keeping it simple)
          // Actually, I should probably include the full content if I am replacing the block.
          // BUT, to be safe and concise, I will assume the user has the code or I can just use the short version for now.
          // However, the "m1", "m2" etc. need to be defined if I want the example to work.
          // I will include the full example 3 content from my previous read.
      ...
      if (!combined.find(f => f.id === "example-flow-1")) combined.push(exampleFlow1);
      if (!combined.find(f => f.id === "example-flow-2")) combined.push(exampleFlow2);
      if (!combined.find(f => f.id === "example-flow-3-v2")) combined.push(exampleFlow3);

      combined.sort((a, b) => b.timestamp - a.timestamp);
      setFlows(combined);
      localStorage.setItem(FLOWS_STORAGE_KEY, JSON.stringify(combined));

      // 4. Initialize State based on Props + Flows
      if (initialMapId === 'new') {
        setNodes(initialNodes);
        setNodeTypes(defaultNodeTypes);
        setHistory([{ nodes: initialNodes, types: defaultNodeTypes }]);
        setHistoryIndex(0);
        setCurrentFlowName("Novo Mapa");
        setCurrentFlowId(null);
      } else if (initialMapId) {
        const foundFlow = combined.find(f => f.id === initialMapId);
        if (foundFlow) {
        ...
        const foundFlow = lastFlowId ? combined.find(f => f.id === lastFlowId) : null;
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    // Calculate relative mouse position (stable for the duration of the event processing)
  ...
  // ... skipping search logic till search
    ...
    <div className="h-full w-full flex relative overflow-hidden bg-[radial-gradient(circle_at_center,_hsl(0_0%_8%)_0%,_hsl(0_0%_4%)_100%)]">
      ...
      <div className="absolute top-6 right-6 z-50 flex items-center gap-3">
        {onBack && (
          <Button
            variant="ghost"
            className="h-10 px-4 gap-2 rounded-lg bg-card/80 border border-border/30 hover:bg-muted text-muted-foreground hover:text-foreground backdrop-blur-sm mr-2"
            onClick={onBack}
          >
