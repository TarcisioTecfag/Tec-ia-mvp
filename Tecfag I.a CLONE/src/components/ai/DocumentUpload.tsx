...
interface DocumentUploadProps {
    ...
    folderId?: string | null; // Pasta onde o arquivo será salvo
...
export function DocumentUpload({ catalogId, folderId, onUploadComplete }: DocumentUploadProps) {
    const [uploadingFiles, setUploadingFiles] = useState<UploadingFile[]>([]);
    const [deliveryMode, setDeliveryMode] = useState<'text' | 'media'>('text');
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        for (const file of acceptedFiles) {
            ...
            try {
                ...
                // Adicionar folderId apenas se fornecido
                if (folderId) {
    ...
    }, [catalogId, folderId, deliveryMode]);
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Delivery Mode Selector */}
            <Tabs value={deliveryMode} onValueChange={(v) => setDeliveryMode(v as 'text' | 'media')} className="w-full">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                {deliveryMode === 'text'
                    ? '📚 O conteúdo será processado e lido pela IA para responder perguntas.'
                    : '📎 O arquivo será entregue como anexo, sem processar o conteúdo.'}
            </p>
