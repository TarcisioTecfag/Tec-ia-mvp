...
import { Settings, Zap, BookOpen, GraduationCap, Briefcase, ChevronRight, Check } from "lucide-react";
...
const modes: { id: ChatMode; label: string; icon: any; color: string; description: string }[] = [
    ...
    },
    {
        ...
        description: "Análise profunda de documentos"
    },
    {
        id: "educational",
        label: "Educativo",
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute bottom-full mb-3 left-0 w-72 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 backdrop-blur-xl"
                    >
                                ...
                                >
                                    ...
                                    <div className="flex-1 text-left">
                                        <div className={cn("text-sm font-medium flex items-center justify-between", mode.color)}>
                                            {mode.label}
                                            {mode.id === "casual" && (
                                                <span className="ml-2 text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded border border-green-500/20 uppercase tracking-widest font-bold">
                                                    Recomendado
                                                </span>
                                            )}
                                            {currentMode === mode.id && (
