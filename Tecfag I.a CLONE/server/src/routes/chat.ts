...
import * as cacheService from '../services/ai/cacheService.js';
...
chatRouter.post('/archive', authenticate, async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Clear user-scoped query cache
        try {
            await cacheService.invalidateCacheByUser(userId);
            console.log(`[Archive Chat] Query cache cleared for user ${userId}`);
        } catch (cacheError: any) {
            console.warn(`[Archive Chat] Failed to clear query cache: ${cacheError.message}`);
        }
