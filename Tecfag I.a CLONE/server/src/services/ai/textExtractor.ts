 ...
 */
async function extractFromPDF(filePath: string, options: { fixEncoding?: boolean } = {}): Promise<ExtractionResult> {
    try {
        const dataBuffer = await fs.readFile(filePath);
        const data = await pdf(dataBuffer);

        return {
            text: cleanText(data.text, options.fixEncoding),
            metadata: {
                pages: data.numpages,
 ...
 */
async function extractFromDOCX(filePath: string, options: { fixEncoding?: boolean } = {}): Promise<ExtractionResult> {
    try {
        const result = await mammoth.extractRawText({ path: filePath });

        return {
            text: cleanText(result.value, options.fixEncoding),
            metadata: {}
        };
    } catch (error) {
 ...
 */
async function extractFromTXT(filePath: string, options: { fixEncoding?: boolean } = {}): Promise<ExtractionResult> {
    try {
        ...
        if (markdown !== text) {
            return {
                text: markdown, // JSON conversion usually produces clean ASCII/UTF8 tables, skip cleaning/fixing?
                // Actually if the JSON content itself had mojibake, we might need to fix it.
                // But tryParseJsonToMarkdown returns a constructed table.
                metadata: { type: 'json-table' }
            };
        }

        return {
            text: cleanText(text, options.fixEncoding),
            metadata: {}
        };
    } catch (error) {
 ...
 */
function cleanText(text: string, fixEncoding: boolean = false): string {
    let result = text;
...
/**
 * Repairs "Mojibake" (garbled text) causing specific character issues
 * Example: "Ã£" becoming "ã", "Ã©" becoming "é"
 * This usually happens when UTF-8 is decoded as ISO-8859-1 (Latin1)
 */
export function repairMojibake(text: string): string {
    try {
        // Safe check: Only attempt if we see suspicious sequences? 
        // Or just apply blindly since user requested "Fix"?
        // We will trust the explicit user action.
        // Convert 'binary' (latin1) to 'utf-8'
        return Buffer.from(text, 'binary').toString('utf-8');
    } catch (e) {
        console.warn('Failed to repair encoding:', e);
        return text;
    }
}
